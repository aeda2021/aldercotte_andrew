...
## A: Because it is bounded by 0 (doesn't allow for negative values), and the mean is low (3)
...
hist(rbinom(500, 100, .5), breaks=20)

# Q: what is plotted on the x axis exactly, in a histogram from rbinom?
## A: # of successes in x trials given probability p (in my case 100 and .5, respectively)
...
model <- lm(fitness ~ body.size, data=soay)
summary(model)
...
## A: No, the QQ plot lines up very poorly (suggesting over- or under-dispersion of residuals compared to a normal distribution). 
## The scale-location plot is showing a positive relationship between the mean and the variance, so the residuals are heteroskedastic
...
## A: Because it's a count data with a mean not far from 0.
...
## The AIC is lower for the model that assumes the error distribution family that fits better, and becuase it uses the log link it
## it isn't bounded by 0.
...
#AA: We are comparing the number of points in each quadrant to see if there is a relationship between gender and survival
...
by_class <- titanic %>% group_by(Pclass) %>% summarise(meanSurv = mean(Survived))
by_class
...
odds = exp(3.2946 + 3*(-0.9606)+ 1*(-2.6434))
odds
odds/(1+odds)
## A: About a 10% chance of survival 
