...
## A: The data do appear reasonably linar on first inspection
...
## A: Yes, almost, but, wt does look a little funky at the high end

# Q: can you explain how the wt distribution might differ from a normal distribution, just by looking at the qq plot?

## A: It looks like the wt data may have a long right tail
...
## A: Hard to say with so few points, but it seems to be fat tailed.
...
## A: Many, but not all, of the QQ plots are more linear looking, but the histograms are all over the place really and I wouldn't
## say that mtcars$wt is exceptional
...
## A: It's pretty unlikely that there isn't a correlation between vehicle weight and fuel efficiency. There is a significant negative
...
##A: Correlation tells us whether variables co-vary, and the p-value tells us the likelihood of getting the sign of our relationship,
## given the data we have, if there was actually no relationship. However, it didn't assign values to the relationship, so we haven't
## really asked how much of the variation in mpg is attributable to weight.
...
## A: You can get more information out of the model object by wrapping it in different functions such as summary or anova,
## and if you save the regression model as an object you can do that at any time, or use it in plots etc.
...
##: A: Residuals vs Fitted doesn't look to bad for a normal distribution, but there are the named outliers at either end. They also show
## up above the 1-1 line in the QQ plot, which otherwise looks good. THere doesn't appear to be any relationship between mean and variance
## in the Scale-Location Plot. The same 3 cars are outliers in the residulas vs leverage plot, but generally the other high-leverage
## data points are pretty close to the mean.
...
## A: The slope is a little lower, and the intercept a little higher, than what I had guesstimated. DF is correct
...
## A: Becuase typing long numbers again and again is a pain, and I'm also definitely retarded enough to put them in the wrong way more
## than half the time...
...
## A: For 95% confidence intervals we want the columns upper and lower. Fitted for the regression line.
...
## A: The 95% confidence interval is for the mean, not for the spread of data.
...
## A: Regression suggests that, on average, an increase in vehicle weight by one ton decreases the fuel efficiency by roughly 5.3
## miles per gallon. 
...
## A: You could say that a vehicle with 0 weight would be expected to have a fuel efficiency of roughly 37 mpg, but that is not very
## useful. It would be more useful to report the mean or median. However, to determine the expected fuel efficiency of a vehicle you
## would multiply it's weight times the coefficient (-5.3) and subtract that from the intercept (37.3)
...
## A: Each group appears to have roughly equal variance. Normal distribution of data points in each group would be easier to visualize with
...
## A: I suspect that the samples were not drawn from underlying populations with equal average sepal widths, and that in fact the average
## sepal width of Iris setosa is the widest, and that I. virginica might even be significantly wider than I. versicolor.

# second, run model
# Q: write code below to run an lm model for sepal width by species
# call your output iris_mod

iris_mod <- lm(Sepal.Width ~ Species, data=df)
...
## A: I think the QQ plot and maybe even the Residuals vs Fitted are kind of interpretable, but lost on the other two.
...
## A: They all look pretty normally distributed to me, so I'd say we are good to go.
...
## A: It's the mean sepal width of the reference species, I. setosa. 

# Q: what do the t values mean, for both the estimate (reference level) and the other comparisons? 

## A: So the t values are the number of SE's the estimate lies from some other value. For the reference species, it's comparing the
## mean sepal-width to 0, for each other species it is measuring the distance between that species mean and the reference mean.

# Q: what does the F and its p value tell you?

## A: At least one species differs significantly from one of the others in mean sepal width

anova(iris_mod) # remember this is how we get anova-type results out of the model we ran
# Q: why are the df what they are? 

## A: DF = 147, which is 150 observations less the three variables observed.

# Q: what do the two MS values tell you?

## A: It looks like it is saying that the with-in group variance is much smaller than the between group variance.
...
# A: All the adjusted pairwise p-values are less than 0.05, so I would say that the population mean sepal width
## of each species is actually different.
...
iris_means <- iris %>% group_by(Species) %>% summarise(mean(Sepal.Width))
iris_means

## A: The differences between means produced by each method are the same. 
...
## A: All of the coefficients change, because the reference level has been changed. However, the R^2
## the F-stat and the p-value for the model as a whole remain the same, because they aren't sensitive
## to which level is used as the reference level. 
...
## A: I'm guessing I did something different from what you expected earlier on, because I already had
## the means calculated and save to iris_means...
...
## A: These iris species do have different sepal widths, at least in the populations sampled. Our analysis
## found the difference between each species' mean sepal widths to be significantly different
## (all pairwise p-values < 0.01)
...
## The initial box-plot depicts the spread of the data, whereas the new plots of the model fits depict
## the estimate of the mean. Whereas individual measurements for any species may vary widely and there is
## overlap in the range of values for each species, the estimate of the means for each species are separate
## and their confidence intervals do not interlap.
...
flowers <- X3_FlowerColourVisits  # substitute your own file path here
...
## R didn't bring mine in as factors
...
## A: Nothing jumps out at me as really obviously being violated...
...
## A: It looks like the effect of color and size on their own will be unclear, but that there is an interraction between the two. 
...
## If two variables are correlated, changes in the level of 1 are associated with changes in the level of the other. 
## If two variables have an interaction, the effect of one variable on a 3rd outcome variable is dependent on the level of
## the second variable. 
...
## A: I think the levene test is telling us that the variances of each group are not significantly different.
...
## A: That the sample sizes are so small that there is no way you could tell whether the distribution is 
## normal by just eye-balling it. 
...
## A: To be honest I didn't really expect color and size to have a significant effect on visitation
## on their own
...
## A: Oh, I guess this explains what I said in answer to the last question?
...
# A: If we are comparing this to the earlier dot plot, that was nearly unintelligible. Now I can see what the likely means of
## the visitation for the underlying population are. My conclusion of this analysis is that there is a relationship
## between color and visitation rate at flowers, but that the relationship is different for flowers of different sizes.
...
## A: The residuals vs fitted plot doesn't show any clear violation of the assumptions, the residuals
## look like they could be normally distributed and are pretty similar across fitted values. There does look
## like there may be more models that expected with a lower than average MPG in the middle of the fitted value range.
## The QQ plot looks OK with the exception of the 3 car models that were also outliers in our initial regression
## I don't see anything interesting in the scale-location plot. The leverage plot is hard to interpret, but it 
## looks like the Maserati Bora is an outlier with a lot of leverage (but Cook's D < 1). I imagine it's HP is
## really large for its weight.
...
## A: I think this is merely a reflection of the units chosen... a change in vehicle weight of 1 ton is related to a larger
## decrease in efficiency than a power increase of 1 HP.
...
## A: Both car weight and horsepower are correlated with fuel efficiency. At a given horsepower an increase in
## weight by one ton is associated with a 3.9 mpg decrease in efficiency. At a  given weight, an increase in power
## of 1 HP is associated with a .03 mpg decrease in efficiency. 
...
## A: I think that egg laying decreases at higher densities, and is lower in the summer than it is in the spring
...
## A: Looks like about -1/30 and they are roughly parallel.

# Q: do you think the effect of density depends on season? that is, do you predict an interaction? explain why or why not
## A: I'm pretty sure there isn't an interaction... The intercept for the relationship between eggs and density
## does depend on season, but the coefficient does not. 
# Q: estimate the intercept(s).  can the intercept be meaningfully interpreted in this model?
## A: The intercepts are the estimates for egg laying at the lowest possible density of limpets in each
## season. They are about 2.4 and 1.6 eggs per limpet for spring and summer respectively.
...
## A: The same, the two are equivalent in R
...
## A: So in the first plot I'm seeing if the residuals are related to the predictors, to see if the assumption of independence there is held.
## In the QQ plot I'm checking for normal distribution of the data at each value of X, by seeing whether the residuals are normally distributed
## In the scale location plot, i'm checking if variance is equal across the range of predictor values
## In the leverage plot I'm looking to see if any single data points are having an oversized effect on the results.
## I don't see anything really wrong... there is a funny little wave in the QQ plot in the middle...
# fourth, interpret results
...
## A: EGGS = 2.66 - .033*DENSITY (Not that the units would go into this equation, but I'm not sure what the density units are) 
...
## A: EGGS = (2.66 - .81) - (.033 -.003)*DENSITY = 1.85 - 0.03*DENSITY
...
## A: They refer to the position in the list of coefficients where each coefficient is found!
...
## A: Ok, so at low densities each limpit lays on average 1.85 eggs (per unit time?) in the summer, and
## 2.66 eggs in the spring. As limpet density increases by 1, the average number of eggs laid decreases by about
## 0.03.
...
## A: The intercept should change, but the slope, p-value and R should not
...
## A: I'm unclear on the correct way to describe this. In a sense we've made HP unit-less, but the units
## are actually '# of SD's'. It allows comparison of data collected with different units.
...
## A: No, I think the intercept and slope will differ, but the p values and R^2 will be the same...
mtcars$hp_standardized = scale(mtcars$hp, center=TRUE, scale=TRUE) 
car_mod5 = lm(mpg ~ hp_standardized, data = mtcars)
summary(car_mod5)
# Q: were you right in your predictions above?
## A: I was wrong, doh, I forgot that we weren't standardizing MPG
...
## A: It's essentially 0, and the p value is 1 because any other mean would be further from 0
...
summary(car_mod2)
## A: The slopes are closer to each-other in value, and they are both less than 1.
...
## A: Using z scores allows you to compare the strength of the effects of two variables measured in different units,
## at least for the range of those variables present in the population you sampled. This can't be done in original units,
## unless you have some clear understanding or expectation about how those two variables relate to each other numerically.
...
## A: I got a significant positive correlation on the 9th try.
# Q: what do you predict will happen if you repeat the exercise with sampling only 5 values? 100 values?
## A: I think I'll be fairly likely to get a significant positive correlation as or more quickly with 5 values
## whereas getting there will take significantly longer with 100 values
...
## A: We are seeing that the smaller the sample, the more likely that sample's mean will be futher from
## from the underlying population's mean. Therefore, there is a higher likelihood of detecting an effect
## (i.e. a low p-value) when there isn't one with a smaller sample size.
...
## A: It has a much narrower spread
# Q: was this what you expected? why or why not?
## Yeah, because the data points are all drawn from a distribution with the same mean, so the mean of the data points should be closer to the underlying mean
## than many of the points that go into them each time. 
# Q: what do you predict will be the effect of increasing sample_size?  ie, how will the histogram change
## A: It should get even narrower, as the likelihood of getting a sample with a mean far from the actual mean decreases
# Q: what do you predict will be the effect of increasing number_samples?  ie, how will the histogram change
## A: I don't think it will change the shape a whole lot, I think it will just affect the range of the y scale.
...
## They were, in the same way that increasing your sampling effort shouldn't affect the shape of your distribution
## for the underlying data points. Increasing the size of a sample decreases the variance in the means,
## but it does not change the variance in the data.
...
## A: The CLT says that if you have a normally distributed population, then if you take samples of that
## population and calculate the means, those means will be  normally distributed.
# Q: what do you predict will happen when we change the distribution to a uniform one?
## A: I actually think the means will still be normally distributed, but a bit wider.
...
## A: Interesting thought experiment, I get that the mean of any numbers in a range will be closer to the middle
## of the range, and so the bigger the sample size the closer to the middle of that range the mean will be.
## I think of it as the probability of rolling different values on two die, where there are a lot more ways to roll central values like 7 than there are
## to role values towards the extremes
