...
## A: They are remarkably similar in my case, with very high count in the mid-2010's
...
## A: They mean for the resample is a few years after the mean for the original set.

# Q: what would happen if you sampled without replacement?

## A: You would get exactly the same distribution as the original (if you still sampled 50 times)
...
means <- pennies_resample %>% group_by(replicate) %>% summarise(x = mean(year))

plot(means$x ~ means$replicate)
hist(means$x, breaks=30)
...
## A: Because each value in the sample can be selected anywhere from 0 to 50 times (but is more likely to be selected 0 to a few times),
## the mean can vary regardless of the limited number of values in the original set.
...
## The distribution of means will still be more a or less normal, but with a little more weirdness to it.
...
medians <- pennies_resample %>% group_by(replicate) %>% summarise(x = median(year))

plot(medians$x ~ medians$replicate)
hist(medians$x, breaks=30)

ggplot(data = medians, aes(x=x)) +
  geom_histogram(binwidth = 1, color = "white") +
  geom_vline(xintercept = CI$l, color = "green") +
  geom_vline(xintercept = CI$u, color = "green")
...
## It retains the values but moves them to different locations. In this case the sum of visits in a year was retained, and visit counts
## were re-assigned to different plant species randomly. 
...
## A: That depends on the question being asked, I guess. In this case we aren't interested in testing whether overall abundance of Xylocopa
## changed randomly or not over the years, so I think it makes sense for them to be maintained. 

rowSums(bee_prefs)
rowSums(bee_prefs_perm)
# Q: are row sums maintained?  should they be?

## A: Row sums are not maintained. This makes sense, the random model we want to test against is whether X. virginica plant visitation
## is significantly different from what kind of visitation might be expected with no preference. 
...
## A: The function above does the following 10 thousand times:
## 1st: it samples a single number of daily visits to M fistulosa from the observed distribution of daily visits to M fistulosa
## 2nd: it then samples from the distribution of single-visit pollen depositions once for each visit on that site day
## 3rd: it sums the number of pollen grains deposited for all visits on the simulated site-day
## The resulting simulated site-day pollen deposition sums are saved as a vector.

# Q: why did we use size=visit_draw?

## A: This propagates error better, because each visit is assigned it's own pollen deposition from the distribution of single visit pollen
## depositions. 
...
## A: Gamma was the best fit based on AIC, which surprised me a little bit because I really thought it looked like a good fit for a 
...
sd_pol
shape_pol <- (mean_vis/sd_vis)^2
shape_pol
scale_pol <- sd_vis^2 / mean_vis
scale_pol

hist(rgamma(173, shape=shape_pol, scale=scale_pol), breaks = 50)

## A: That still doesn't look like a great fit tbh.
...
for (i in 1:10000) {
  ...
  pollen_draws1 <- rgamma(visit_draw1, scale = scale_pol, shape = shape_pol)
...
##A: R returned 100 means of 66 bee weights selected from the distribution of bee weights WITH replacement.
...
## The data mean falls right towards the middle of the confidence interval for the bootstrapped means.
...
## The quantities in the stat column are the difference between the mean dry weight for workers and queens in each permutation.

# Q: how will you use these numbers? what are we trying to test here?

## A: These numbers give a distribution of differences in queen and worker mean weights that might be found if their is no relationship 
## between weight and case. We can compare the distribution of these values to the one we actually measured to find the likelihood of our
## measurement under the null assumption. 
...
## A: It's close to zero, as would be expected by randomizing assignment of weights to castes. The means of two random samples from the same
## population should be similar each time, and normally distributed around the population mean.
...
## A: The difference between means in our sample is about 18, which is much larger than the differences in the 1st 10 random permutations
## Therefore, I expect we'll reject our null hypothesis by a wide margin. 
...
## A: The bootstrapped CI's give us the range of means we might expect to measure for the whole population of queens and workers combined
## However, the means for each caste would likely fall outside of the range of means expected for the whole population.
...
null_dist %>%
  ...
  shade_p_value(obs_stat = 1, direction = "two-sided")
...
get_p_value(null_dist, 1, direction = "two-sided")

get_p_value(null_dist, 2, direction = "greater")
