...
plotPost(fm.crabs.dat$b, main = 'Effect of Salinity')
plotPost(exp(fm.crabs.dat$alpha), main = 'Estimated # of Burrows at Mean Salinity')
...
*Answer:*
* The expectd number of burrows at mean salinity is 10.9, and the95% confidence interval is from 9.58 to 12.6
* If we hadn't centered salinity on the mean, we would likely have an intercept that signified 0 salinity (fresh water), which may not be be within the realm of interesting values for this crab species. 
* The effect of salinity is positive, with change in log(burrows) of 0.0339 for each 1 SD increase in salinity. The 95% confidence interval on this effect is from 0.0201 to 0.0474
* Given the posterior distribution of our effect, it appears that there is almost no chance that salinity has no effect on burrow count. 
...
tick.mod <- function(){
 for(i in 1:N){
  y[i] ~ dbern(p[i])
  logit(p[i]) <- bj[site[i]] + b1*x1[i] + b2*x2[i] + b3*x3[i]
  }
  
  ## Random intercepts:
  for(j in 1:n.sites){
    # each site-level intercept is a realization of a normal process:
    bj[j] ~ dnorm(b0, 1/sigma^2) 
  }
...
fm.ticks <- jags(
  data = datList.ticks,
  model.file = tick.mod,
  parameters.to.save = c('b0','b1','b2','b3','sigma'),
  n.chains = 3,
  n.iter = 20000,
  n.burnin = 5000,
  n.thin = 3
...
plotPost(fm.ticks.dat$b1, main = 'Effect of Forested Area')
plotPost(fm.ticks.dat$b2, main = 'Effect of Forest Edge')
plotPost(fm.ticks.dat$b3, main = 'Effect of Evergreen-Deciduous Ratio')
plotPost(fm.ticks.dat$sigma, main = 'Variance')
plotPost(plogis(fm.ticks.dat$b0), main = 'Mean Probability of Detecting E. chaffeensis')
```
...
*Answer:*
The mean occupancy of EC was 0.278. It's highly probable that an increase in forested area is associated with a decreased likelihood of EC occupancy. Similarly an increase in the amount of forest edge decreases EC occupancy. However, whether forest cover is evergreen or deciduous does not have a clear effect on the likelihood of detecting EC. It seems that the reality is counter to your hypotheses? Large, fragmented forests have lower probability of EC, and coniferoues and deciduous forests are roughly equivalent?